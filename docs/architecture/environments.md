...
- **staging**: protected Cloudflare Worker and dedicated Supabase project containing
  synthetic data only.
- **production**: separate Cloudflare Worker and Supabase project; not configured or
  approved for deployment by Milestone 4 planning.
...
ProtoTower.ai is reserved for the public product but is not connected. Milestone 4
remains an invite-only synthetic staging scope. Staging continues to use the
documented Workers development hostname until a separate production and DNS decision
is approved.
