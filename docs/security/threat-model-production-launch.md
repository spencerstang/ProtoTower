# Quiet-launch production beta threat model
...
| Automated or abusive account creation                | Keep email confirmation required, retain provider rate limits, add edge abuse controls, monitor non-sensitive failure counts, and retain a rapid signup-disable procedure. |
...
| A production incident cannot be contained            | Document contacts, disable signup/email first, roll back the Worker without reversing migrations, use forward-only fixes, and preserve redacted evidence.                  |
...
Residual risks include possession-based email authentication without MFA, public
account creation, manual deletion, reliance on Cloudflare/Supabase/Resend, and the
hosted backup lifecycle. These are accepted for the quiet-launch beta and must be
